...
public class FileCheckTask {
	...
	public void checkFile() {
		...
		List<Path> fileListPaths = oldList.stream()
										  .map(dto -> Paths.get("c:\\upload",dto.getUploadPath(),dto.getUuid(),dto.getFileName()))
										  .collect(Collectors.toList());
		
		//썸네일 이미지 경로 추출
		oldList.stream()
				.filter(dto -> dto.isFileType() == true)
				.map(dto -> Paths.get("c:\\upload",dto.getUploadPath(),"s_"+dto.getUuid(),dto.getFileName()))
		...
		//targetDir에 접근 후 파일 목록을 추출하여 데이터베이스 파일목록과 일치하지 않은 
		//파일 정보 담기
